...
# unicodedata_db.js, unicodename_db.js, unicodetype_db.js
#
# history:
...
# 2016-07-28 qsantos  convert output from C to Javascript
#
...
def makeunicodedata(unicode, trace):
    ...
    print(file=fp)
    print('var UNIDATA_VERSION = "%s";' % UNIDATA_VERSION, file=fp)
    print("/* a list of unique database records */", file=fp)
    print("var _PyUnicode_Database_Records = [", file=fp)
    for item in table:
        print("    [%d, %d, %d, %d, %d, %d]," % item, file=fp)
    print("];", file=fp)
    print("/* Fields indexes for _PyUnicode_Database_Records */", file=fp)
    print("var DB_CATEGORY=0;", file=fp)
    print("var DB_COMBINING=1;", file=fp)
    print("var DB_BIDIRECTIONAL=2;", file=fp)
    print("var DB_MIRRORED=3;", file=fp)
    print("var DB_EAST_ASIAN_WIDTH=4;", file=fp)
    print("var DB_NORMALIZATION_QUICK_CHECK=5;", file=fp)
    print(file=fp)

    print("/* Reindexing of NFC first characters. */", file=fp)
    print("var TOTAL_FIRST =",total_first, file=fp)
    print("var TOTAL_LAST =",total_last, file=fp)
    print("var nfc_first = [", file=fp)
    for start,end in comp_first_ranges:
        print("  [%d, %d, %d]," % (start,end-start,comp_first[start]), file=fp)
    print("];\n", file=fp)
    print("var nfc_last = [", file=fp)
    for start,end in comp_last_ranges:
        print("  [%d, %d, %d]," % (start,end-start,comp_last[start]), file=fp)
    print("];\n", file=fp)
    print("/* Fields indexes for nfc_first and nfc_last */", file=fp)
    print("var NFC_START=0;", file=fp)
    print("var NFC_STOP=1;", file=fp)
    print("var NFC_INDEX=2;", file=fp)
    print(file=fp)
    ...
    print("var _PyUnicode_CategoryNames = [", file=fp)
    for name in CATEGORY_NAMES:
        print("    \"%s\"," % name, file=fp)
    print("];", file=fp)

    print("var _PyUnicode_BidirectionalNames = [", file=fp)
    for name in BIDIRECTIONAL_NAMES:
        print("    \"%s\"," % name, file=fp)
    print("];", file=fp)

    print("var _PyUnicode_EastAsianWidthNames = [", file=fp)
    for name in EASTASIANWIDTH_NAMES:
        print("    \"%s\"," % name, file=fp)
    print("];", file=fp)

    print("var decomp_prefix = [", file=fp)
    ...
    print("var DECOMP_SHIFT =", shift, file=fp)
    Array("decomp_index1", index1).dump(fp, trace)
    Array("decomp_index2", index2).dump(fp, trace)

    index, index2, shift = splitbins(comp_data, trace)
    print("/* NFC pairs */", file=fp)
    print("var COMP_SHIFT =", shift, file=fp)
...
def makeunicodetype(unicode, trace):
    ...
    print("/* One record is made of: upper, lower, title, decimal, digit, flags */", file=fp)
    print("var _PyUnicode_TypeRecords = [", file=fp)
    for item in table:
        print("    [%d, %d, %d, %d, %d, %d]," % item, file=fp)
    print("];", file=fp)
    print("/* Fields indexes for _PyUnicode_TypeRecords */", file=fp)
    print("var TYPE_UPPER=0;", file=fp)
    print("var TYPE_LOWER=1;", file=fp)
    print("var TYPE_TITLE=2;", file=fp)
    print("var TYPE_DECIMAL=3;", file=fp)
    print("var TYPE_DIGIT=4;", file=fp)
    print("var TYPE_FLAGS=5;", file=fp)
    print(file=fp)

    print("/* extended case mappings */", file=fp)
    print(file=fp)
    print("var _PyUnicode_ExtendedCase = [", file=fp)
    ...
    print(' */', file=fp)
    print('function _PyUnicode_ToNumeric(ch)', file=fp)
    print('{', file=fp)
    ...
    print(" */", file=fp)
    print('function _PyUnicode_IsWhitespace(ch)', file=fp)
    print('{', file=fp)
    print('    switch (ch.charCodeAt(0)) {', file=fp)
    ...
    print(" */", file=fp)
    print('function _PyUnicode_IsLinebreak(ch)', file=fp)
    print('{', file=fp)
    print('    switch (ch.charCodeAt(0)) {', file=fp)
...
def makeunicodename(unicode, trace):
    ...
    print("/* this file was generated by %s %s */" % (SCRIPT, VERSION), file=fp)
    print(file=fp)
    print("var NAME_MAXLEN =", 256, file=fp)
    print(file=fp)
    ...
    print("var phrasebook_shift =", shift, file=fp)
    print("var phrasebook_short =", short, file=fp)
    ...
    print(file=fp)
    print('var aliases_start = %#x;' % NAME_ALIASES_START, file=fp)
    print('var aliases_end = %#x;' %
          (NAME_ALIASES_START + len(unicode.aliases)), file=fp)

    name_aliases = [codepoint for name, codepoint in unicode.aliases]
    Array("name_aliases", name_aliases).dump(fp, trace)

    print('var named_sequences_start = %#x;' % NAMED_SEQUENCES_START, file=fp)
    print('var named_sequences_end = %#x;' %
          (NAMED_SEQUENCES_START + len(unicode.named_sequences)), file=fp)

    print('var named_sequences = [', file=fp)
    for name, sequence in unicode.named_sequences:
        seq_str = ', '.join('0x%04X' % cp for cp in sequence)
        print('    {seqlen: %d, seq: [%s]},' % (len(sequence), seq_str), file=fp)
    print('];', file=fp)
...
class Hash:
    ...
    def dump(self, file, trace):
        ...
        file.write("var %s_magic = %d\n" % (self.name, self.magic))
        file.write("var %s_size = %d\n" % (self.name, self.size))
        file.write("var %s_poly = %d\n" % (self.name, self.poly))
...
def splitbins(t, trace=0):
    ...
    if trace:
        def dump(t1, t2, shift, bytes):
            print("%d+%d bins at shift %d; %d" % (
                len(t1), len(t2), shift, bytes), file=sys.stderr)
        print("Size of original table:", len(t), file=sys.stderr)
